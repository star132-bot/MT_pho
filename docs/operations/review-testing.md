...
The secret-free Super Admin batch acceptance was re-run on 2026-07-31 at `1440 x 1000` and `390 x 844`. It selected two eligible owned untouched submissions, required one explicit attestation covering all ten visible policy checks, fetched each current Detail, issued two dedicated self-publish requests with independent request keys, verified the single-item ten-check shortcut, and confirmed the queue heading sits above the batch toolbar without overlap or clipped button text. Mobile controls remained inside the toolbar and viewport, no console/page errors were reported, and the named browser session closed. This test uses only a loopback fake provider and never mutates development or production data.
