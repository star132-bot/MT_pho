...
- Sessions: exchange the Supabase session at the application boundary for server-managed `HttpOnly`, `Secure`, `SameSite=Lax` cookies. Admin roles require Supabase MFA; an ordinary user may opt in to TOTP, after which every protected account/workspace surface requires AAL2 until that user explicitly disables the factor from an AAL2 session.
...
The application sets `redirect_to` from the fixed `MT_PUBLIC_BASE_URL`; it never builds email callbacks from untrusted request payloads or non-loopback Host headers. Supabase Auth must allow the exact `/auth/verify-email` and `/auth/reset-password` callback URLs. Registration requires matching 12-128 character passwords and records the accepted Terms/Privacy policy version in Auth metadata. Register, resend-verification, and recovery requests use bounded per-email and per-IP application limits in addition to Nginx/provider controls; resend is enumeration-safe. Default Supabase implicit email links are supported by immediately exchanging the fragment refresh token for server-managed HttpOnly cookies and scrubbing the URL without browser storage. Production email templates should prefer first-party fragments containing `token_hash` and `type`, for example `/auth/verify-email#token_hash={{ .TokenHash }}&type=signup` and `/auth/reset-password#token_hash={{ .TokenHash }}&type=recovery`, so the application can POST the one-time hash to Supabase `/verify` without exposing access/refresh tokens to page code or request logs.
...
Google and Apple sign-in use the Supabase OAuth broker with a server-owned PKCE flow. MT Presence generates a high-entropy verifier and a one-use ten-minute opaque state, sends only the SHA-256 challenge to `/auth/v1/authorize`, and exchanges the returned code from the BFF. Only the Supabase access/refresh session is written to application HttpOnly cookies; provider tokens are discarded. The callback accepts only a server-recorded internal destination, verifies the returned provider identity, and then applies the same verified-email, active-account, role, and MFA rules as password sign-in. Google and Apple client credentials live only in Supabase provider configuration. Account identity linking uses the authenticated `/auth/v1/user/identities/authorize` flow with a separate state purpose bound to the initiating user; the callback requires the provider session to resolve to that same user before the application session is refreshed. Current single-process state is intentionally process-local; a later multi-instance Web tier must move the ten-minute OAuth flow record to a shared expiring store before horizontal activation.

Supabase's verified TOTP factor is the sole source of truth for an ordinary account's 2FA preference; MT Presence does not maintain a duplicate boolean in the product database. Enroll is incomplete until a successful challenge/verify upgrades the session to AAL2. A verified factor makes password and OAuth sign-ins enter the same MFA continuation, and the server rejects AAL1 requests to protected account, workspace, communication, and review APIs even if the browser hides no controls. Disabling requires CSRF, an explicit confirmation, and a current AAL2 session; the server revokes other device sessions before unenrolling every TOTP factor. Admin and Super Admin accounts always remain MFA-mandatory and cannot use the disable operation.
