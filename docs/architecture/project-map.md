...
- 2026-08-04：邮箱注册验证主流程改为 6 位 OTP。注册与验证码重发进入 `/auth/verify-email`，新增独立 `/api/auth/verify-email-code`、10 分钟验证码尝试桶及 Nginx 登录级限流；服务端使用 Supabase `type=email` 校验 OTP，验证成功后签发既有 HttpOnly 应用会话。旧 signup link callback 保留兼容，Fake Supabase 回归覆盖验证码格式拒绝、正确 OTP 建立会话和 Cookie。
