...
- 入口：`/auth/sign-in`、`/auth/oauth/google`、`/auth/oauth/apple`、`/auth/oauth/callback`、`/auth/register`、`/auth/resend-verification`、`/auth/forgot-password`、`/auth/reset-password`、`/auth/verify-email`、`/auth/mfa`、`/dashboard`、`/settings/account`。
- 主要用户操作：注册与邮箱验证、邮箱密码/Google/Apple 登录、登录/退出、找回与重置密码、普通用户自选 TOTP、管理员强制 TOTP、查看受保护个人资料、编辑 creator Profile/Authorship Preferences、连接或移除登录身份、查看当前 Session、退出当前/其他/全部设备。
...
- `server.py`：Supabase Auth/PostgREST 代理、规范化邮箱、注册/重发/找回邮件限流、密码确认、固定 callback、Google/Apple OAuth 服务端 PKCE、区分 login/link 的一次性 10 分钟 OAuth state、access/refresh rotation、CSRF/Origin、recovery grant、strict Profile/cover allowlist、Account/Workspace route guard、统一 TOTP/AAL2 会话门禁和 Session scope revoke。身份连接 API 绑定发起用户、验证回调用户一致性，并只向浏览器输出 provider/id/email/timestamp allowlist；移除操作验证 owner 且至少保留一个身份。OAuth provider token 不进入 Cookie/HTML/日志；登录与 OAuth 回调检查 verified email、active account、server-derived roles 和已验证 TOTP，普通 AAL1 会话只允许进入 `/auth/mfa` challenge，管理员与启用 2FA 的普通用户都 fail closed。`GET /api/auth/mfa/status`、`GET /api/auth/mfa/factors`、`POST /api/auth/mfa/enroll|challenge|verify` 和受 CSRF 保护的 `DELETE /api/auth/mfa` 复用 Supabase Auth 因子边界。Recovery OTP 成功以 provider 返回的 session/user 为准；Supabase 的 OTP AMR 不被误要求为 `recovery`，应用通过独立 HttpOnly marker 将该会话限制在重置流程，密码写入仍要求 10 分钟 server-side grant，重置、显式登录或退出后清除 marker。
...
- `scripts/test_auth_security_boundary.py` / `scripts/test_oauth_identity_boundary.py` / `scripts/test_user_dashboard_boundary.py`：loopback fake provider 的 Account 与 creator profile/cover 集成回归，不使用真实凭据或远程账号；Google/Apple fixture 验证 PKCE challenge/verifier、一次性回调、开放重定向拒绝、provider credential 不落浏览器、link mutation 的 CSRF/发起用户绑定、身份投影、owner unlink 和最后身份保护；Recovery fixture 使用 Supabase 实际的 `otp` AMR，验证应用 marker 仍会阻断 Workspace 与 Account Settings，直到密码成功重置。
...
- Security：只读显示 verified email、服务端角色、account status 和当前 AAL；Authenticator app 使用 Supabase 已验证 TOTP factor 作为唯一启用状态，普通用户可进入 QR setup 或在当前 AAL2 会话中确认停用，管理员始终 Required；Linked accounts 独立呈现 Email/Google/Apple 的已连接状态、连接命令和移除确认，至少一个身份时才允许移除；任何已启用 TOTP 的 AAL1 会话都跳转 MFA。
...
- API：`GET/PATCH /api/me/profile`、`GET/PATCH /api/me/profile/cover`、`GET /api/me/identities`、`POST /api/me/identities/link`、`DELETE /api/me/identities/{identity_id}`、`GET /api/me/sessions`、`DELETE /api/me/sessions/{others|all}`、`GET /api/auth/mfa/status`、`GET /api/auth/mfa/factors`、`POST /api/auth/mfa/{enroll|challenge|verify}`、`DELETE /api/auth/mfa`；所有 mutation 要求 JSON、same-origin、CSRF Cookie/header 双提交。Factor response 只投影 id/type/status/friendly name，enrollment secret 仅在新建 factor 时返回；Identity response 不返回 `identity_data`、provider token 或内部诊断；Cover response 只返回固定 asset DTO 与短期 signed URL，不返回 bucket/key/owner/scan internals。
...
- 2026-08-10：把既有 Admin TOTP 扩展为统一的可选账户 2FA。Supabase 已验证 TOTP factor 是唯一启用状态，不新增易漂移的业务布尔值；密码、Google、Apple OAuth 与受保护页面/API 在启用后统一拒绝 AAL1，只允许进入 `/auth/mfa` enrollment/challenge。Account Settings Security 新增 Authenticator app On/Off/Required 状态、可访问 switch 与停用确认；普通用户停用要求当前 AAL2、CSRF，并先撤销其他设备，管理员不能停用。Factor 列表只输出 allowlist，不返回 secret/provider internals；fake-provider 回归覆盖 enrollment、错误/正确验证码、会话轮换、密码/OAuth 绕过拒绝和安全停用。Review 静态契约同步验证统一 MFA 守卫，完整 release gate 通过。
- 2026-08-07：在既有邮箱密码与 Google 登录基础上新增 Apple OAuth，Google/Apple 共用服务端 PKCE callback、一次性 state、verified email、active account、Admin MFA 与登录成功默认 `/works.html` 边界，provider token 仍不进入 Cookie、HTML 或日志。Account Settings 的 Security 新增 Linked accounts：服务端通过 authenticated identity authorize API 发起手动连接，state 绑定发起用户并在回调核对同一 user id；浏览器仅收到 provider/id/email/timestamp allowlist，移除前确认 owner 且至少保留一种登录方式。Auth 桌面与 390px 移动端视觉检查通过；新增 secret-free Apple/login/link/unlink 集成回归并纳入 release gate。生产启用前仍须在 Apple Developer 与 Supabase Auth Provider 中配置凭据及准确 callback URL。
- 2026-08-06：按 Google 品牌识别重做 Auth 的 Google 登录按钮：单色字母圆章改为官方四色 G，文案统一为 “Sign in with Google”；入口调整到邮箱密码表单与 `OR` 分隔线之后，浅灰胶囊按钮延长为与 430px 表单容器同宽，并补齐 hover、active 与键盘 focus-visible 状态。OAuth URL、服务端 PKCE、回调和登录成功跳转保持不变，桌面与移动端继续要求无横向溢出。
