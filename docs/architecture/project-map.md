...
- `lightbox.html`：访客浏览器本地 Lightbox；使用全站统一公开 rail，展示、移除、清空个人选择，并把所选作品上下文带入 Contact。
...
- `account-settings.html` / `account-settings.js`：受保护 `/settings/account` 账户页面；提供 Profile、Authorship Preferences、Security 和 Sessions 四段信息架构，维护表单 dirty/disabled/saving/error/success 状态，使用 `/api/me/profile` 读写服务端 profile，并通过 provider 支持的 `others` / `all` scope 撤销会话；不伪造 Supabase 无法提供的远程设备列表或位置历史。
- `styles.css`：全站视觉系统和响应式布局；定义纯白 gallery palette 色彩 token、`--ui-*` 组件状态/动效 token、字体、间距、按钮、导航、SVG 图标、双层首页图片覆盖过渡、四段图文 Statement、Infinite Marquee Gallery、全部主要公开页与 Upload/Review/Account 共用的 78px rail、顶部搜索与 tabs、masonry 图片墙、图标式 hover 操作层、沉浸式作品查看器、Upload 渐进式布局/Loading/Queue/readiness/提交确认/移动端状态、联系页、Account Settings 表单/会话/确认弹窗、统一 focus-visible、gallery stagger reveal 和移动端规则。
...
- `server.py`：Python 标准库本地开发服务器；提供静态文件与 legacy SQLite Archive API；Supabase 边界覆盖 Auth/MFA/Profile/Session 和 Workspace 路由保护；Workspace 提供 Folder、signed upload、Draft、readiness 与 Submit API，所有 mutation 先做 CSRF/session/active account/Admin AAL2/输入 allowlist；Readiness 与 Submit response 都做严格字段投影，Submit 要求 confirmation、`expected_version` 和 UUID idempotency key；legacy Archive 非公开读取与 mutation 继续仅允许 Admin+AAL2。
...
- `scripts/deploy_supabase_phase1.sh`：Phase 0/1 Supabase 数据库部署入口；使用 libpq `PG*` 环境变量避免密码出现在进程参数，fresh database 默认执行 baseline 后按文件名顺序执行 `database/migrations/*.sql`；已有数据库使用 `MT_APPLY_PHASE1_BASELINE=no` 只执行幂等增量 migration；默认拒绝未确认的 production 部署。
...
- Gallery 提供 Search、Type、Ratio 三组叠加过滤器，并把状态写入 URL；公开页面不提供 Arrange。
...
- `works.html`：公开 rail、Search/Type/Ratio、Count、gallery、空态/toast 和带 Info、Add to Lightbox/Inquire/Download/Related Works 的沉浸式 Viewer；公开 UI 不含 Upload、Review、Arrange。
...
- `styles.css`：公开 rail、搜索/tabs、masonry、hover、Viewer/Info、Series/About/Lightbox/Contact、内部工具和响应式规则。
...
- 过滤：顶部 Search、Type、Ratio 叠加生效；状态同步到 `?q=`、`?type=`、`?ratio=`，Count 使用 `aria-live` 更新；公开页面不提供 Arrange。
- Gallery：四列 masonry 保持自然比例；hover 只显示品牌角标、Add to Lightbox 和 Download；Lightbox 状态写入 `mt-presence-lightbox-v1`，Draft/Archived 记录不能进入公开列表。
...
## 4. Series、About 与 Lightbox

### 功能说明

- `collections.html` 只负责作者策展的公开 Series；`about.html` 负责作者实践与专业信息；`lightbox.html` 只负责访客当前浏览器中的个人选择。
- 页面入口：`collections.html`、`about.html`、`lightbox.html`。
- 主要用户操作：筛选和打开 Series、按作者顺序观看系列作品、进入 Works Viewer、管理 Lightbox、携带 Work/Series/Lightbox 上下文咨询。

### 相关文件

- `collections.html` / `collections.js`：Series index/detail、状态筛选、statement、明确作品顺序、query-string detail、下一系列和 inquiry。
- `series-data.js`：静态阶段 Series 定义；每个系列明确列出 workIds。
- `about.html`：Practice、Availability 和 Contact CTA。
- `lightbox.html` / `lightbox.js`：本地选择清单、移除/清空、空态、Works Viewer 和 Contact 入口。
- `public-archive.js`：三类页面共享 published archive 读取和 Lightbox key 迁移。
- `styles.css`：定义 `.series-*`、`.about-*`、`.lightbox-*` 和公开页面响应式规则。

### 页面内部结构

- Series：All/Ongoing/Completed 筛选；`?series={slug}` 直达详情；作品链接使用 `works.html?work={id}&from=series&series={slug}`。
...
- 状态：API 不可用时显示 sample fallback；空 Series/Lightbox 有单一有效下一步；所有公开页桌面/移动端无横向溢出。
...
- 响应式：Works 桌面使用 rail、其余公开页使用 header；内部工具移动端隐藏 rail，页面不保留空白宽度。
...
- `works.html`：公开作品只读取 published；rail 的 Upload 入口进入独立的受保护 Workspace，不把 Draft 暴露到公开作品流。
- `manage.html`：仍是 legacy SQLite Review/Publish 原型；Phase 2E 可以创建 Supabase `review_submissions`，但该页面尚未读取它，Admin Review Queue 是下一切片。
...
- Phase 2A-2F 已把用户 Folder、Draft、Version、三类 Asset metadata、private Storage、可信扫描、自动保存、乐观并发、readiness 与 Submit transaction 接入 Workspace；Admin Review Queue/decision、Publish、公开 delivery 与 sample migration 尚未接入同一生产链路。
...
- 后续生产切片先让 Admin Review Queue/Detail 读取 Phase 2E 的 `review_submissions`，再实现 decision/Publish 与 published-only delivery；之后迁移 `works.html`、sample metadata、首页精选和需要保留的标签/切片能力。
...
- 后续展示：Admin Review Queue/decision/Publish 通过后把 approved display/thumbnail 发布到 public delivery，再让公开 Works 只读取 `publication_status = published` 的稳定 DTO；原图保持 private。
...
- 入口：`/auth/sign-in`、`/auth/register`、`/auth/forgot-password`、`/auth/reset-password`、`/auth/verify-email`、`/auth/mfa`、`/settings/account`。
- 主要用户操作：注册与邮箱验证、登录/退出、找回与重置密码、Admin TOTP、编辑 Profile/Authorship Preferences、查看当前 Session、退出当前/其他/全部设备。
...
- `account-settings.html` / `account-settings.js`：Account Settings 页面、Profile/Preferences 表单、Security 摘要、current-only Session 列表、确认弹窗、dirty 状态、field error 和 bulk revoke。
- `server.py`：Supabase Auth/PostgREST 代理、access/refresh rotation、CSRF/Origin、recovery grant、Profile allowlist、Account/Workspace route guard、Admin role+AAL2 和 Session scope revoke。
...
- `scripts/test_auth_security_boundary.py`：loopback fake provider 集成回归，不使用真实凭据或远程账号。
- `scripts/test_supabase_deploy_script.py`：fresh/incremental 部署顺序回归。

### 页面内部结构

- Profile：`display_name`、`bio`、HTTPS `website_url`、二字母 `country_code`；client、server 和 SQL RPC 使用一致边界，未知字段拒绝。
...
- API：`GET/PATCH /api/me/profile`、`GET /api/me/sessions`、`DELETE /api/me/sessions/{others|all}`；所有 mutation 要求 JSON、same-origin、CSRF Cookie/header 双提交。
