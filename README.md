...
- Protected Upload Studio with server-authoritative Supabase Folders and Drafts, a two-worker upload queue, per-item Cancel/Retry/Remove controls, shared-declaration Quick Upload for multi-file batches, current-Folder batch submission of server-confirmed Ready Drafts, owner-scoped signed uploads to three private Storage buckets, canceled-object cleanup, 900 ms debounced Draft autosave, version-conflict recovery, five-check submission readiness, idempotent Submit for Review, soft-delete Trash/Restore views, and read-only IndexedDB offline cache.
...
- Supabase email, Google OAuth, and Apple OAuth Register/Verify/Sign In/Sign Out/Forgot/Reset flow with server-side PKCE, HttpOnly session cookies, CSRF protection, owner isolation, user-selectable authenticator 2FA, mandatory Admin MFA, and linked-account management.
- Protected Account Settings with a real owner-scoped profile-photo upload/remove flow, five creator-profile groups and ten editable identity/work/location/about/link fields, plus authorship preferences, verified account state, an Authenticator app On/Off/Required control, current-session description, and provider-supported bulk session revocation. Source photos are center-cropped and re-encoded in the browser; only a private 512x512 JPEG is uploaded.
...
Copy `.env.example` values into your local environment before starting the server. Set `MT_PUBLIC_BASE_URL` to the exact browser origin and add `/auth/verify-email`, `/auth/reset-password`, and `/auth/oauth/callback` to the Supabase Auth redirect allowlist. Registration requires a normalized email, a matching 12-128 character password confirmation, and explicit acceptance of the current Terms/Privacy policy; verification can be requested again without revealing whether an account exists. Google and Apple login use the same server-side PKCE flow; their client credentials live only in Supabase, and provider tokens never enter browser storage or application cookies. Account Settings > Security can link or unlink configured provider identities while preventing removal of the final sign-in method, and ordinary users can enable authenticator 2FA. A verified TOTP factor forces every later password or provider login through `/auth/mfa`; disabling it requires the current AAL2 session, CSRF confirmation, and revocation of other devices. The auth routes use Supabase Auth through the server, keep access/refresh tokens in `HttpOnly` cookies, require a same-origin CSRF token for mutations, and never use browser storage for credentials. Production email delivery additionally requires Supabase Confirm Email, a verified custom SMTP sender, and an end-to-end mailbox test. `/dashboard` and `/workspace/images` are protected, `/workspace` canonicalizes to Dashboard, direct `/upload-studio.html` requests canonicalize to Upload Studio, and Admin/Super Admin sessions always require AAL2 before opening these account surfaces.
...
- `scripts/test_auth_security_boundary.py`: secret-free local integration test for CSRF, account recovery, profile read/write, optional TOTP enrollment/verification/disable, password/OAuth MFA enforcement, session revocation, Workspace/Account route guards, mandatory Admin MFA, and private legacy originals.
